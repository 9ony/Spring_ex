...
public class CartController {
	...
	public String addCart(Model m, HttpSession session,
		...
		UserVO loginUser=(UserVO)session.getAttribute("loginUser");//세션에서 UserVO가져옴
		...
		//장바구니에 상품 추가
		//log.info("idx ===>"+cvo.getIdx_fk());
		//List<CartVO> cartArr=shopService.selectCartView(cvo.getIdx_fk());//해당회원 장바구니
		//log.info("cartArr===>" + cartArr);
		//m.addAttribute("cartArr",cartArr);
		// 여기서 forward이동하면 /cartAdd가 계속 호출되면서 장바구니 상품이 계속 증가하는 현상
		// 그래서 redirect로 이동해야한다
	...
	public String cartList(Model m, HttpSession session) {
		UserVO loginUser=(UserVO)session.getAttribute("loginUser");//세션에서 UserVO가져옴
		int idx_fk=loginUser.getIdx();
		List<CartVO> cartArr=shopService.selectCartView(idx_fk);//해당회원 장바구니
		log.info("cartArr===>" + cartArr);
