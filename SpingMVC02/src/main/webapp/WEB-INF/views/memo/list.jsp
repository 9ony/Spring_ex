...
<c:if test="${memoArr ne null and not empty memoArr }">
	...
	<tr>
		<td><b>${memo.idx }</b></td>
		<td><b>${memo.msg }</b><span class="float-right"> [<fmt:formatDate pattern="yyyy-MM-dd" value="${memo.wdate}"/>]</span></td>
		<td><b>${memo.name }</b></td>
		<td><b><a href="memoEdit?idx=${memo.idx }">수정</a>|<a href="memoDel?idx=${memo.idx }">삭제</a></b></td>
	</tr>
	</c:forEach>
</c:if>
	<tr>
	<td colspan="4">총 게시글수 : ${totalCount }</td>
	</tr>
</table>
</div>
</body>
